...
public class LandlineDecoderIndia extends LandlineDecoderAbstract {

  boolean areaCodePopulated = false;
  HashMap<String, String> mpAreaCodeRegion = new HashMap<String, String>(2600);

  @Override
  public String getUserCountryTreePath() {
    return "Earth/Asia/India";
  }


  private void populateAreaCodes() {
      readFile("RegionsAndLandline_Asia_Bharat.txt");
      String line = null;
  ...
  private void processLine(String line) {
    ...
    countryCode = ShieldConstants.COUNTRY_CODE_BHARAT+countryCode;
  ...
  public void getCallerInfo(RequestObject ro){
     ...
     if(!areaCodePopulated){
      ...
      ro.setStrTreepathCallerLocation("Asia/India");
      ro.setStrCallerAreaCode(ShieldConstants.COUNTRY_CODE_BHARAT);


    String phoneNumber = ro.getStrCallerPhoneNumber();

    if (phoneNumber.startsWith("0")) {
      phoneNumber = phoneNumber.replaceFirst("0", ShieldConstants.COUNTRY_CODE_BHARAT+"");
    }
    else if (phoneNumber.startsWith("+"+ShieldConstants.COUNTRY_CODE_BHARAT)) {
